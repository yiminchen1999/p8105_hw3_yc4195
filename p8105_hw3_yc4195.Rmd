...
The ny_noaa dataset contains informationaccessed from the NOAA National Climatic Data Center. The ataset has `r nrow(ny_noaa)` rows and `r ncol(ny_noaa)` columns.The variables in this dataset are `r names(ny_noaa)` .
`id`: the Weather station ID
...
`snow`: Snowfall (mm)
`snwd`: Snow depth (mm)
The extent to which missing data is an issue is determined by whether the data is missing at random since the station may not collect some precipitation or snow data.
